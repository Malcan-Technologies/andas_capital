...
# SSL Setup Script for DocuSeal - Final Fixed Version
...
if [ ! -d "$DOCUSEAL_DIR" ]; then
    ...
    echo "📁 Available directories in $USER_HOME:"
    ls -la "$USER_HOME" | grep docuseal || echo "   No docuseal directories found"
    exit 1
fi
...
echo "🐳 Updating Docker Compose to support HTTPS..."

# Create a simple script to update docker-compose.yml
...
try:
    ...
    # Update nginx service to mount SSL certificates and expose port 443
...
(crontab -l 2>/dev/null | grep -v certbot; echo "0 12 * * * /usr/bin/certbot renew --quiet --deploy-hook 'cd $DOCUSEAL_DIR && sudo -u $ACTUAL_USER docker-compose restart nginx'") | crontab -
