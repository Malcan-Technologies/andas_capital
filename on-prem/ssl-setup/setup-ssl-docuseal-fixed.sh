...
# SSL Setup Script for DocuSeal - Fixed Version
...
DOCUSEAL_DIR="$HOME/docuseal-onprem"
...
# Read current docker-compose.yml and add SSL certificate mounts to nginx service
python3 << 'PYTHON_EOF'
import yaml
import sys
...
(crontab -l 2>/dev/null | grep -v certbot; echo "0 12 * * * /usr/bin/certbot renew --quiet --deploy-hook 'cd $DOCUSEAL_DIR && docker-compose restart nginx'") | crontab -
