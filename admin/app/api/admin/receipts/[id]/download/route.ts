...
export async function GET(request: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/api/admin/receipts/${receiptId}/download`, {
      method: 'GET',
      headers: {
        'Authorization': token,
      },
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { success: false, message: errorData.message || 'Failed to download receipt' },
    ...
    console.error('Error in download receipt API route:', error);
    return NextResponse.json(
